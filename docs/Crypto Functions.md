...
| :-------------: | :--------------------: | :----------: | :---------------: | :--------------: |
| **Mandatory:sym** | The symbol of interest |symbol        | If an empty symbol is passed, all syms are chosen | \`BTCUSDT |
| exchanges       | The exchanges to be queried |symbol        | If an empty symbol is passed, all exchanges are chosen | \`finex |
| timestamp       | The time to subtract the window from | timestamp     | If proctype is rdb, default time is the last time data is received. If proctype is hdb, default time is the last time data was received for the previous day | 2020.04.16D09:40:00.0000000 |
| window          | The amount of time from the timestamp to look at | second        | Twice as long as the frequency of querying data (.crypto.deffreq) | 00:00:30 |
...
| :-------------: | :--------------------: | :----------: | :---------------: | :--------------: |
| **Mandatory:sym** | The symbol of interest |symbol        | If an empty symbol is passed, all syms are chosen | \`BTCUSDT |
| date            | The date to retreive data for |date        | If an empty symbol is passed, all exchanges are chosen | \`finex |
| quote           | The bid and/or ask columns required by the user | symbol     | If an empty symbol is passed, all bid and ask columns are chosen | \`bid |
| byexchange      | Allows user to filter ohlc data at individual exchange level | boolean | 0b | 1b |
...
| :-------------: | :--------------------: | :----------: | :---------------: | :--------------: |
| **Mandatory:sym** | The symbol of interest |symbol        | If an empty symbol is passed, all syms are chosen | \`BTCUSDT |
| exchanges       | The date to retreive data for |date        | If an empty symbol is passed, all exchanges are chosen | \`finex |
| starttime       | The time at which to begin looking at data | timestamp     | If proctype is rdb, the start of day is used. If proctype is hdb, start of previous day is used | 2020.04.16D09:40:00.000000 |
| endtime         | the time at which to stop looking at data | timestamp | If proctype is rdb, the query time is used. If proctype is hdb, the end of previos day is used.  | 2020.04.16D12:00:00.000000 |
| bucket          | What bucket of time to group data by | second | 00:01:00 | 00:02:00 |
